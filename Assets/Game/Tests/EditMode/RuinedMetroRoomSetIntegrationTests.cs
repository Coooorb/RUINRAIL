...
{
    /// <summary>
    /// TASK 087: the complete Ruined Metro set feeds the biome pool with exact counts, every seeded graph + assembly
    ...
    /// </summary>
    public class RuinedMetroRoomSetIntegrationTests
    {
        private const string SummaryPath = "TestResults/roomset_ruined_metro_summary.md";
        ...
        public void SetUp()
        {
            ...
            var metro = RoomValidationTools.LoadAllRoomDefinitions().Where(d => UnityEditor.AssetDatabase.GetAssetPath(d).StartsWith(RuinedMetroRoomSet.DefinitionFolder)).ToList();
        ...
        [Test]
        public void MetroPool_HasExactlyTwentyOneRooms_WithTheApprovedDistribution_AndNoRejects()
        {
            ...
            Assert.IsTrue(_pool.Rooms.All(r => r.Biome == Biome.RuinedMetro && r.Prefab != null && r.SupportedDoors.Count > 0));
        }
        ...
        public void InvalidRoom_NeverEntersThePool()
        {
            ...
            _created.Add(broken);
            typeof(RoomDefinition).GetField("_id", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(broken, "metro_broken_99");
            typeof(RoomDefinition).GetField("_biome", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(broken, Biome.RuinedMetro);
            ...
            var pool = RoomPool.Build(_pool.Rooms.Append(broken), Biome.RuinedMetro);
            Assert.AreEqual(21, pool.Rooms.Count);
            Assert.AreEqual(1, pool.Rejected.Count);
            Assert.AreEqual("metro_broken_99", pool.Rejected[0].RoomId);
        }
        ...
        public void ManySeedsAndDepths_AssembleValidDungeons_WithCategorySlotsRespected_AndBoundedReuse()
        {
            ...
            summary.AppendLine("# Ruined Metro room-set validation summary");
            summary.AppendLine();
