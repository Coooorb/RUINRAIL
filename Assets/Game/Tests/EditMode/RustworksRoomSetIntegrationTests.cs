...
{
    /// <summary>
    /// TASK 113: the complete Rustworks set feeds the biome pool with exact counts, every seeded graph + assembly
    ...
    /// </summary>
    public class RustworksRoomSetIntegrationTests
    {
        private const string SummaryPath = "TestResults/roomset_rustworks_summary.md";
        ...
        public void SetUp()
        {
            ...
            var rust = RoomValidationTools.LoadAllRoomDefinitions().Where(d => UnityEditor.AssetDatabase.GetAssetPath(d).StartsWith(RustworksRoomSet.DefinitionFolder)).ToList();
            _pool = RoomPool.Build(rust, Biome.Rustworks);
        }
        ...
        [Test]
        public void RustworksPool_HasExactlyTwentyOneRooms_WithTheApprovedDistribution_AndNoRejects()
        {
            ...
            Assert.IsTrue(_pool.Rooms.All(r => r.Biome == Biome.Rustworks && r.Prefab != null && r.SupportedDoors.Count > 0));
        }
        ...
        public void InvalidRoom_NeverEntersThePool()
        {
            ...
            _created.Add(broken);
            typeof(RoomDefinition).GetField("_id", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(broken, "rust_broken_99");
            typeof(RoomDefinition).GetField("_biome", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(broken, Biome.Rustworks);
            ...
            var pool = RoomPool.Build(_pool.Rooms.Append(broken), Biome.Rustworks);
            Assert.AreEqual(21, pool.Rooms.Count);
            Assert.AreEqual(1, pool.Rejected.Count);
            Assert.AreEqual("rust_broken_99", pool.Rejected[0].RoomId);
        }
        ...
        public void DuplicateRoomId_ExcludesEveryDefinitionSharingIt_WithAClearReport()
        {
            ...
            var pool = RoomPool.Build(_pool.Rooms.Append(duplicate), Biome.Rustworks);
            ...
            Assert.IsEmpty(RoomPool.Build(_pool.Rooms, Biome.Rustworks).Rejected, "The authored set itself has no duplicates.");
        }
        ...
        public void ManySeedsAndDepths_AssembleValidDungeons_WithCategorySlotsRespected_AndBoundedReuse()
        {
            ...
            summary.AppendLine("# Rustworks room-set validation summary");
            summary.AppendLine();
