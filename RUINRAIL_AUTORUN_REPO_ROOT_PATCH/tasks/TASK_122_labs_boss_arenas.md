...
- TASK 121 must be complete or independently audited as already satisfied.
- Preserve the confirmed implementation state from all earlier completed tasks.
- If an earlier dependency is missing or broken, stop and report the dependency failure instead of inventing a substitute architecture.

## GOAL

Author the Overgrown Labs Boss arena room-prefab subset using the validated room pipeline.
...
1. Create exactly 2 Boss arenas; every prefab is tagged Overgrown Labs, has a unique stable room ID and correct category.
...
1. Exactly 2 Boss arenas validated Overgrown Labs rooms exist in this subset.
...
In normal single-task mode, stop before TASK 123. Under `production/130_AUTONOMOUS_TASK_RUNNER.md`, write the checkpoint to the autonomous run log and continue only if the runner's gate rules permit it.
