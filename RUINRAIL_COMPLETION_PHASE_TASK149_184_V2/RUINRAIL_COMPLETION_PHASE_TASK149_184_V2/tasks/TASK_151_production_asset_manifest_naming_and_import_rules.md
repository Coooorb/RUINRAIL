...
- TASK 150 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Create the final production asset pipeline/validation rules so external or generated art/audio can enter Unity consistently without damaging pixel fidelity or runtime references.
...
1. Extend the asset manifest with exact role IDs and expected source/runtime paths based on the existing project layout; do not invent a parallel root if one already exists.
2. Define deterministic naming/slicing/import metadata for world sprites, sprite sheets, tiles, UI sprites, fonts and audio while preserving existing conventions.
3. World pixel sprites must preserve 32 PPU where the approved spec requires it, hard pixel edges and no accidental smoothing/mipmap artifacts.
4. Add editor validation that detects missing role IDs, duplicate role mappings, invalid world PPU/import settings, broken sprite references and placeholder assets mistakenly marked approved.
5. Track source/originality/license metadata for every final third-party or generated asset.
6. Provide a narrow replacement workflow so final assets can replace placeholders without changing gameplay IDs.
...
1. A new approved asset can be added/replaced without touching gameplay code.
2. Validator reports actionable errors for missing/duplicate/bad-import/placeholder-as-final cases.
3. Existing content IDs and serialized references remain stable.
4. Full test harness passes.
...
In single-task mode, stop before TASK 152. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
