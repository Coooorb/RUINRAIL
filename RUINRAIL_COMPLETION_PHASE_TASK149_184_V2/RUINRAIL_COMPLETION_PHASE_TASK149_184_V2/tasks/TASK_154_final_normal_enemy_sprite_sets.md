# TASK 154 — Final Normal Enemy Sprite Sets
...
- TASK 153 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Integrate final original sprite sets for all 9 normal enemy archetypes with distinct gameplay-readable silhouettes.
...
1. Integrate final source art for Grunt, Shooter, Swarm, Charger, Brute, Bomber, Shield, Sniper and Summoner.
2. Silhouette must communicate role before surface detail; shield/frontality, sniper long weapon, brute mass, swarm scale and bomber payload must remain clear.
3. Telegraph source art must support the already-implemented attack timings; never change attack timing to accommodate frames.
4. Validate sorting, facing and scale in rooms from every biome.
5. Remove release-path placeholder fallbacks for these nine archetypes.
...
1. All 9 normal enemies have approved final source art mapped to manifest roles.
2. Attack/readability checks survive dark/bright room variants.
3. No gameplay/balance changes.
...
In single-task mode, stop before TASK 155. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
