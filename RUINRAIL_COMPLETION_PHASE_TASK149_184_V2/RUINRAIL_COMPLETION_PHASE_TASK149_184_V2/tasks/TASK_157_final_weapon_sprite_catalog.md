# TASK 157 — Final Weapon Sprite Catalog
...
- TASK 156 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Integrate final original sprites for all 33 weapons while preserving class identity, equipped alignment and Legendary differentiation.
...
1. Integrate exactly the existing 33 weapon definitions: 22 regular + 11 Legendary; do not add variants.
2. Every weapon has a distinct readable silhouette while same-class weapons share family language.
3. Validate grip/pivot/muzzle alignment through all aim directions and common animation states.
4. Legendary weapons must read as special without becoming visually incompatible with RUINRAIL.
5. Map world/equipped/inventory usage from one approved source where practical instead of unrelated illustrations.
...
1. 33/33 weapon visual roles resolve to final approved sources.
2. No weapon uses missing/placeholder sprite in world, equipped or UI path where that representation is required.
3. Aim/pivot/muzzle alignment is correct.
4. Weapon behavior/balance remains unchanged.
...
In single-task mode, stop before TASK 158. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
