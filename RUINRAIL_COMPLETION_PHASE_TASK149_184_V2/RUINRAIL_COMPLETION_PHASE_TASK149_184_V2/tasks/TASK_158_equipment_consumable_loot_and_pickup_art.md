# TASK 158 — Equipment, Consumable, Loot and Pickup Art
...
- TASK 157 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Complete final presentation for armor, accessories, consumables, ammo, coins, rarity/loot and world pickups used by the existing game.
...
1. Integrate final icon/world-pickup art for the existing 9 armor, 16 accessories, 10 consumables, 4 ammo categories, coins and required generic loot containers/markers.
2. Use one-item-per-slot/inventory semantics already implemented; art must not imply nonexistent equipment slots or currencies.
3. Rarity presentation must support Common/Uncommon/Rare/Epic/Legendary without color-only communication.
4. Legendary loot receives a clear special read compatible with the existing VFX/audio hooks.
...
1. Every runtime/UI item role in the manifest has a final representation or an explicitly approved shared representation.
2. No icon misrepresents slot/type/ammo category.
3. Color-blind/readability rules remain satisfied.
...
In single-task mode, stop before TASK 159. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
