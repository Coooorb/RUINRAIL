# TASK 162 — Safehouse, Transit and Base Environment Art
...
- TASK 161 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Integrate final art for the Safehouse/Base stations and expedition transit while preserving station interactions and scene flow.
...
- `ui/94_BASE_UI.md`
- `base/safehouse specifications used by current implementation`
...
1. Final Safehouse should feel warmer, maintained and inhabited relative to dungeons while still built from salvaged infrastructure.
2. Visually identify Storage, Loadout, Trader, Character, Workshop, Multiplayer Terminal and Expedition Transit without relying only on floating text.
3. Transit car must read as heavy, mechanical and dependable.
4. Preserve interaction positions, station IDs and base flow.
...
1. All mandatory base stations and transit presentation roles are final.
2. Navigation and interaction readability is improved without gameplay changes.
3. No placeholder environment art remains in release Base scene.
...
In single-task mode, stop before TASK 163. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
