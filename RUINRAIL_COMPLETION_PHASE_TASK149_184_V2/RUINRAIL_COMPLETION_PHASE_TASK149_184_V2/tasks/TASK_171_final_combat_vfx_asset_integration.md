# TASK 171 — Final Combat VFX Asset Integration
...
- TASK 170 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Replace placeholder/generic combat effects with final RUINRAIL VFX assets while reusing the proven pooled feedback architecture.
...
- `existing EffectPool/CombatFeedback/TelegraphIndicator implementation`
...
1. Integrate final muzzle/impact/projectile/tracer/explosion/melee/stagger/heal/status and Legendary-drop effects for the existing combat categories.
2. Differentiate class feel without turning each of 33 weapons into an unrelated visual language.
3. Blaster Heat/Overheat, Rocket explosion, Shotgun weight and Boss attacks need distinct readable feedback.
4. VFX remain presentation-only and pooled; never apply damage.
5. Respect accessibility hooks for shake/flashes.
...
1. Mandatory combat VFX roles are final and mapped.
2. No high-frequency allocation/leak regression.
3. Effects do not obscure hazards/telegraphs at active-count caps.
...
In single-task mode, stop before TASK 172. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
