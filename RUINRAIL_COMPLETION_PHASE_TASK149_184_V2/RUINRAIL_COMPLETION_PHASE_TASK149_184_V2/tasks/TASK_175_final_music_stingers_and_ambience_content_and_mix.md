# TASK 175 — Final Music, Stingers and Ambience Content and Mix
...
- TASK 174 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Integrate the approved V1 music/stinger/ambience scope and verify transitions/mix in the real game.
...
- `existing MusicRoles/MusicDirector/MusicBinder implementation`
...
1. Integrate exactly 11 V1 music roles: Main Menu, Shelter, and Exploration/Combat/Boss for each of 3 biomes.
2. Integrate exactly 6 required stingers: Legendary Drop, Elite Encounter, Boss Defeated, Extraction Success, Expedition Failed, Level Up.
3. Integrate exactly 3 ambience loops: Metro, Rustworks, Labs.
4. Verify crossfades/transitions do not restart incorrectly during room flow and Boss transitions.
5. Ambience remains below gameplay cues.
6. Record source/license metadata.
...
1. 11/11 music roles, 6/6 stingers and 3/3 ambience roles resolve to production audio.
2. No placeholder/silent mandatory role remains.
3. Transitions route correctly across MainMenu/Base/biomes/combat/boss/extraction/failure.
...
In single-task mode, stop before TASK 176. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
