...
> **Pinned baseline:** confirmed against Unity's official 6000.0 package documentation (2026-09-14) and **verified in the real project (2026-09-25)** — see Verification Status.
...
- Reference resolution and pixel rules remain defined in `docs/design/art/101_PIXEL_GRID_AND_SCALE.md`.
...
./scripts/run-unity-tests.ps1 -TestPlatform EditMode -TestFilter "FixtureName"   # targeted
```
...
./scripts/run-unity-tests.sh EditMode "FixtureName;Other.Test"   # targeted (Unity -testFilter)
```
...
- **Actual project resolution:** **VERIFIED (2026-09-25).** `Packages/manifest.json` carries exactly the seven pins above; the project resolves, compiles, runs the full EditMode/PlayMode suites and builds a non-development player under Unity `6000.3.24f1` (see `production/CURRENT_RELEASE_STATUS.md`). No version was changed.
- **Ongoing requirement:** Unity Package Manager must resolve the exact approved pins without silent substitution. If resolution fails, follow the Package Change Rule above and report the exact conflict.
