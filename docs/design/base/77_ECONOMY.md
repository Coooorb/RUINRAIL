...
**Taking Banked Coins into an Expedition** (design update 2026-09-26, explicit owner request): before departure each player may choose, at the Transit station, how many of their own Banked Coins to take (0 up to their whole balance; NONE / ALL and a step selector — the step is a temporary tunable on `EconomyConfig.CarryCoinStep`, not a balance number). Nothing moves while choosing. The expedition start moves exactly the chosen amount, once, from Banked into Carried Coins; from then on they are ordinary Carried Coins — banked again by a successful return, lost on death/failure or an abandoned run. In co-op each player chooses for themselves; the host captures each member's amount at start and holds it as that member's Carried Coins.
