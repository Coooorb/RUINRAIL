...
- **Transit decision** (after the boss, 60): a compact panel at the top centre — depth now > next, best depth and coins at risk, two buttons RETURN TO SHELTER / DESCEND DEEPER (co-op: each shows its vote tally; the status line says this player's vote, who is still deciding, a Dead player's missing vote, or the Dead-teammate Return warning with CONFIRM RETURN / CANCEL). The world keeps running (the Boss Cache is still to open). Mouse: click a button (the pointer on the panel holds gameplay input, so the click never fires). Keyboard / controller: **F** / **D-pad up** takes the panel (gameplay input held; arrows / D-pad select, Enter / A confirm, Esc / B hand it back); a cast vote hands focus back. The panel closes when the decision resolves.
