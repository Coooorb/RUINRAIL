...
The scripts call Unity in batch mode with `-runTests`, write XML/log output to `TestResults/`, and propagate failures through a non-zero exit code. The Unity executable/version is governed by `docs/ENVIRONMENT.md`. Which tests to run when (targeted first, `-testFilter` via the scripts) is the testing ladder in `docs/DEVELOPMENT_WORKFLOW.md`.
