# TASK 163 — Final UI Skin, Icons, Glyphs and Pixel Font
...
- TASK 162 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Replace placeholder UI presentation with the final RUINRAIL utilitarian pixel interface while preserving existing view models and accessibility.
...
1. Skin existing Main Menu, HUD, Inventory, tooltips, Base, multiplayer, pause/settings/rebinding, transit vote, downed/dead and expedition-summary screens; do not rebuild their logic.
2. Integrate final frames, buttons, focus/selection states, icons, ammo/coin/HP/XP/skill glyphs, rarity frames and controller/keyboard glyphs.
3. Integrate a legally usable pixel/readable font and record source/license metadata; headings may be more stylized than body copy.
4. Maintain 640×360 readability and scalable presentation at supported resolutions.
5. Focus states and important warnings must use shape/text/icon in addition to color.
...
1. No release UI screen relies on placeholder frames/icons/font for mandatory content.
2. Controller and keyboard focus remain obvious.
3. Text clipping/layout tests pass at reference and supported scaled resolutions.
4. Accessibility/rebinding behavior is unchanged.
...
In single-task mode, stop before TASK 164. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
