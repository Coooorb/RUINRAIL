# TASK 167 — Final Normal Enemy Animation Production and Integration
...
- TASK 166 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Populate final animation content for all 9 normal enemies while preserving authoritative AI/attack timing.
...
- `existing EnemyAnimationDriver/attack implementations`
...
1. Resolve every required clip role for all 9 normal enemies.
2. Telegraph frames must clearly precede damage according to existing gameplay timing; animation events must not become the sole authority for damage.
3. Keep directional/state coverage defined by the current audit; use shared source structure only where it does not erase archetype identity.
4. Validate interruptions, stagger, death and spawn/despawn transitions.
...
1. Normal-enemy animation audit has zero missing mandatory roles.
2. Telegraphs are readable with sound off.
3. No attack damage timing changes.
...
In single-task mode, stop before TASK 168. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
