# TASK 168 — Final Elite and Boss Animation Production and Integration
...
- TASK 167 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Populate final animation content for all 6 Elites and 6 Bosses, including phase/telegraph presentation required by their existing movesets.
...
- `existing Elite/Boss animation/attack implementations`
...
1. Resolve all manifest-required Elite/Boss clip roles.
2. Prioritize windup/telegraph/phase readability over decorative realism.
3. Boss phase transitions and dangerous attacks must have unmistakable visual preparation consistent with existing timings.
4. Validate every Elite/Boss in representative authored arena/room.
...
1. Elite/Boss animation audit has zero missing mandatory roles.
2. All dangerous attacks remain readable without audio.
3. No behavior/balance changes.
...
In single-task mode, stop before TASK 169. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
