# TASK 169 — Weapon and Interaction Animation Finalization
...
- TASK 168 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Complete final weapon, reload/recoil/charge/overheat/melee and interaction visual animation coverage using the existing weapon visual architecture.
...
- existing WeaponVisualDriver implementation
...
1. Use separate weapon/pivot animation for firearms where existing architecture supports it; do not require unnecessary unique body reload clips.
2. Validate all 11 classes: fire/recoil/reload or class-equivalent action, Bow charge/release, Blaster Heat/Overheat/Vent, Knife/Spear attack motion, Rocket launch and Legendary special visual triggers.
3. Ensure reload/charge/special cancel cases reflect existing authoritative states.
4. Add/finalize interact/revive/use visuals only where the manifest requires them.
...
1. All required weapon/interaction animation roles resolve.
2. Visual state never grants damage/ammo or changes cooldown authority.
3. Melee visuals align with actual reach/arc.
...
In single-task mode, stop before TASK 170. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
