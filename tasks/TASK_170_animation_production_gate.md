# TASK 170 — Animation Production Gate
...
- TASK 169 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Close the TASK-148 animation blocker by proving that all required final animation roles are present, correctly imported, integrated and gameplay-timing safe.
...
- AnimationAssetAudit and TASK 166–169 reports
...
1. Run AnimationAssetAudit and any production animation validator against release scenes/assets.
2. The prior report identified 22 animation sets × 48 clip roles; compare current exact audit output and explain any intentional structural change.
3. Exercise rapid state transitions, death/downed/revive, weapon swap, overheat, charge, Elite/Boss phases and scene transitions.
4. Run full EditMode/PlayMode regression.
...
1. Zero mandatory animation roles missing/placeholder.
2. Zero animation-related runtime exceptions/broken references.
3. Authoritative gameplay timing regression tests remain green.
4. Gate report produced.
...
In single-task mode, stop before TASK 171. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
