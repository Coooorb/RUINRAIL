# TASK 172 — Environment, Loot, Status and UI VFX Integration
...
- TASK 171 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Finalize non-weapon VFX for environment hazards, pickups/rarity, interactables, status states and UI/world feedback.
...
- `existing hazard/loot/UI feedback implementations`
...
1. Finalize biome hazard/ambient effects required by existing gameplay, without turning ambience into collision/damage authority.
2. Finalize loot rarity/Legendary pickup glow, chest/event/interactable feedback and coin/ammo pickup effects.
3. Finalize downed/revive/heal/status/world UI indicators where current systems expose hooks.
4. Keep particles/smoke short and readable.
...
1. All mandatory non-combat VFX manifest roles integrated.
2. Loot/interactables remain visible in all biome palettes.
3. No effect changes gameplay result.
4. Performance/readability tests pass.
...
In single-task mode, stop before TASK 173. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
