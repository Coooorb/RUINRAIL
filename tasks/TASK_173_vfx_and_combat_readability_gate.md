# TASK 173 — VFX and Combat Readability Gate
...
- TASK 172 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Validate final VFX and visual combat readability under realistic high-action conditions and close the VFX presentation blocker.
...
- TASK 171–172 reports
- existing performance stress tests
...
1. Run high-enemy-count/co-op-equivalent stress scenes with final VFX enabled.
2. Verify projectiles, Boss/Elite telegraphs, hazards, loot and players remain distinguishable.
3. Verify reduced/off screen shake and applicable flash settings.
4. Capture representative combat screenshots/video for review and run full regression.
...
1. No critical telegraph is obscured by final effects.
2. VFX pools are stable under stress.
3. Accessibility toggles work without gameplay changes.
4. All VFX manifest roles pass.
...
In single-task mode, stop before TASK 174. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
