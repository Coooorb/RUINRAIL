# TASK 174 — Final Gameplay SFX Content and Mix
...
- TASK 173 must be complete, or the runner must have an explicit approved gate record allowing continuation.
...
Supply/integrate production SFX for every current gameplay-significant audio event while reusing the existing AudioService/event routing.
...
- `existing AudioEvents/AudioService/GameplayAudioBinder implementation`
...
1. Confirm current exact audio-event role count from code; TASK 148 reported 53 unique event IDs. Do not add arbitrary event categories solely to hit a count.
2. Integrate original/licensed production clips for weapon classes, Blaster heat, enemy/Elite/Boss telegraphs, hits, explosions, loot, interactions, transit and UI actions required by the manifest.
3. Use sensible variation/routing where repetition is distracting without changing gameplay semantics.
4. Mix for combat readability: dangerous cues intelligible, ambience/music never masks them, UI subtle.
5. Record source/license metadata for each external/generated clip.
...
1. Every mandatory current gameplay SFX event has production clip coverage or an explicitly approved shared clip mapping.
2. No missing-clip warnings on a representative full run.
3. Audio-off and reduced-volume settings remain functional.
...
In single-task mode, stop before TASK 175. Under the review-gated runner, continue only when the runner and any human/external gate permit it.
